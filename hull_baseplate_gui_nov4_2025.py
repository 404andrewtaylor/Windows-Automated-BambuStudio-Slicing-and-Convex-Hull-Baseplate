    ...
    def _move_output_files(self, stl_file_path):
        ...
        try:
            ...
            if self.keep_intermediate_files.get():
                ...
                # Keep only final output - copy all *_with_hull_baseplate.gcode.3mf files and the log file
                import glob
                ...
                try:
                    shutil.rmtree(pipeline_dir)
                    if not os.path.exists(pipeline_dir):
                        self.log(f"Pipeline folder removed successfully: {pipeline_dir}")
                    else:
                        self.log(f"Warning: Pipeline folder still exists after removal attempt: {pipeline_dir}")
                except Exception as e:
